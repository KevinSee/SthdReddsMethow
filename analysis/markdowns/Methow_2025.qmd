...
format:
  pdf:
    ...
    latex-extra-dependencies: ["placeins"]
...
set_flextable_defaults(theme_fun = 
                         function(ft) {
                           ft |> 
                             # theme_zebra(odd_header = "transparent") |> 
                             theme_zebra(odd_header = "#1B305A",
...
# query and prep data
prep_uc_sthd_data(basin = "Methow",
                  query_year = yr,
...
For non-index reaches, which were surveyed only once during peak spawning, the estimate of total redds was calculated by dividing the observed redds by the estimate of net error associated with that survey (@eq-net-error). This assumes that no redds were washed out before the non-index survey, and that no new redds appeared after that survey. As the number of redds observed in the non-index reaches ranged from `r paste(range(redd_df$new_redds[redd_df$index == 'N']), collapse = ' to ')`, any violation of this assumption should not affect the overall estimates very much. Any index reaches that did not meet the thresholds described above were treated as non-index reaches, and the total observed redds in those reaches were divided by the mean estimate of net error for each reach.
...
The estimated net error, observed redds and estimates of redds at the reach scale are shown in @tbl-redd-est-rch. Plots of the new redds and the GAUC fit to those data are shown in @fig-gauc-plots. The results are summarized by major areas and population scale in @tbl-redd-est-strm. 

```{r}
#| label: tbl-redd-est-rch
#| tbl-cap: Estimates of mean net error and redds for each reach.
#| tbl-pos: "!ht"
...
redd_results %>%
  ...
  select(River = river, 
         ...
         `Redds CV` = redd_cv) %>%
  flextable() |> 
  colformat_double(j = c(4, 5),
                   digits = 3) |> 
  colformat_double(j = c(6, 7),
                   digits = 0) |> 
  colformat_double(j = 8,
                   digits = 1) |> 
  colformat_double(j = 9,
                   digits = 3) |> 
  set_formatter(value = drop_trailing_zeros) |> 
  width(j = c(1, 3),
        width = 1) |> 
  width(j = 2,
        width = 0.6) |> 
...
```{r}
#| label: fig-gauc-plots
#| fig-cap: Plots of observed redd counts (black dots) through time for each qualifying index reach, and the fitted curve from the GAUC model (blue line) with associated uncertainty (gray).
...
```{r}
#| label: tbl-redd-est-strm
#| tbl-cap: Estimate of redds in mainstem Wenatchee, above and below Tumwater Dam, and any tributaries surveyed.
#| tbl-pos: "!ht"
...
Demographic data about sex and origin were derived from the PIT tags detected within each area (@tbl-pit-tag-tab). 

```{r pit-tag-tab}
#| label: tbl-pit-tag-tab
#| tbl-cap: Number of PIT tags detected in each area by sex and origin.
#| tbl-pos: "!ht"
...
Parameter estimates for fish / redd and proportion hatchery based on this PIT tag data are shown in @tbl-fpr-hatch-prop (for areas with redd data) and @tbl-fpr-hatch-prop-other (for other areas). Fish / redd estimates in @tbl-fpr-hatch-prop-other are not used in the analyses presented here, but the results are made available. 

```{r}
#| label: tbl-fpr-hatch-prop
#| tbl-cap: Fish per redd and hatchery origin proportion estimates.
#| tbl-pos: "!ht"
...
fpr_df %>%
  ...
  flextable() |> 
  set_header_labels(location = "Area",
                    fpr = "Fish / Redd",
                    fpr_se = "FpR Std. Error",
                    phos = "Prop. Hatchery",
                    phos_se = "Prop Std. Error") |> 
  colformat_double(j = -1,
                   digits = 3) |> 
  width(j = 1,
        width = 1.5) |> 
...
```{r}
#| label: tbl-fpr-hatch-prop-other
#| tbl-cap: Fish per redd and hatchery origin proportion estimates for areas without redd surveys.
#| tbl-pos: "!ht"
...
low_met_spwn |>
  ...
  flextable() |> 
  # set_table_properties(opts_pdf = list(table.env = "longtable")) |> 
  flextable::bold(i = n_distinct(low_met_spwn$reach) + 1) |> 
  width(j = 1,
        width = 1) |> 
  width(j = 2,
        width = 1.5) |> 
...
```{r}
#| label: tbl-spwn-area
#| tbl-cap: "Estimates (CV) of spawners by area and origin."
...
spwn_strm |>
  ...
  width(j = 1:4,
        width = 1.5) |> 
  bold(i = n_distinct(spwn_strm$river) + 1)
```


## Prespawn Mortality

The estimates of overall prespawn mortality within the Methow population, by origin, are shown in @tbl-ps-mort-met. 

```{r}
#| label: tbl-ps-mort-met
#| tbl-cap: Methow prespawn mortality estimates. Includes estimates (standard error) of escapement, spawners, rate of prespawn mortality, and standard error of this rate, separated by origin.
#| tbl-pos: "!ht"
...
The data for this report was collected by Washington Department of Fish and Wildlife and BioAnalysts, and funded by Douglas County Public Utility District.
...
# Appendix A: Sex Errors at Priest Rapids Dam {#sec-appendix-a}

Fish collected for broodstock at various locations throughout the Upper Columbia (e.g. Wells Hatchery and within the Wenatchee) had a known sex. Based on PIT tag detections of this broodstock collection, those sexes could be compared with the sex call at Priest Rapids Dam when the fish was initially tagged. Assuming that this error rate is systematic across the entire run within a brood year, we combined all tags with this kind of comparison. (@tbl-sex-err) shows the sample sizes and results.
...
```{r}
#| label: tbl-sex-err
#| tbl-cap: Error rate in sex calls at Priest Rapids Dam.
#| tbl-pos: "!ht"

# compile all years of sexing error rates
read_excel(paste("T:/DFW-Team FP Upper Columbia Escapement - General/UC_Sthd/estimates",
...
# Appendix B: Estimates of Steelhead at Priest Rapids Dam {#sec-appendix-b}

Counts at Priest Rapids Dam were adjusted to estimate the number of unique steelhead by origin passing the dam. Since the tags used in the DABOM model are a random sample of the run, we partitioned the total window counts by origin using those tags. Columbia River Research [Data Access in Real Time](https://www.cbr.washington.edu/dart) built a custom query to summarize how many PIT tags were detected moving over Priest Rapids Dam, and how many of them were detected making multiple journeys up the fish ladder. We used this to estimate the re-ascension rate for natural and hatchery origin fish, and multiplied the estimates of fish at Priest Rapids Dam by one minus this re-ascension rate, leading us to estimates of unique dam passage by origin.

In some years, the counts at Priest Rapids Dam have been unreliable (e.g. many more fish counted at the next upstream dam, Rock Island Dam, than at Priest Rapids Dam). In those circumstances, we have estimated the total steelhead by origin at Priest Rapids Dam by starting with counts at Rock Island. We use the same method described above to estimate the number of unique dam crossings at Rock Island by origin. We use the DABOM tags detected at Rock Island or upstream to determine the proportion of hatchery and natural origin fish, and DART built a separate query to estimate the re-ascension rate at Rock Island. We then divide that estimate by the probability that a steelhead has moved from Priest Rapids Dam to Rock Island Dam (from the DABOM model), which is estimated separately for each origin, and that is our estimate of unique steelhead, by origin, that crossed Priest Rapids Dam that year.
