...
rem_df |> 
  pivot_wider(names_from = origin, 
              ...
              values_fill = 0)  |> 
  clean_names("title") |> 
  kableExtra::kbl(caption = "Known number of fish removed at particular sites for surplus or broodstock, or due to harvest, by origin.",
                  booktabs = T) %>%
  kable_styling(bootstrap_options = c("striped", 
                                      "hover"),
                latex_options = c("striped",
...
redd_results %>%
  ...
  select(River = river, 
         ...
         `Redds CV` = redd_cv) %>%
  kableExtra::kbl(digits = c(rep(NA, 3), rep(3, 2), 1, 1, 1, 3),
...
summ_redd_tab %>%
  ...
  rename(`# Reaches` = `Number Reaches`,
         `Redds CV` = `Redds Cv`) |> 
  kableExtra::kbl(digits = c(rep(1, 6), 1, 3),
                  booktabs = T,
                  format.args = list(drop0trailing = T),
...
fpr_df %>%
  ...
  kableExtra::kbl(digits = 3,
                  booktabs = T,
                  caption = "Fish per redd and hatchery origin proportion estimates for areas with redd counts.",
...
fpr_df %>%
  ...
  kableExtra::kbl(digits = 3,
                  booktabs = T,
                  caption = "Fish per redd and hatchery origin proportion estimates for areas without redd surveys.",
                  col.names = c('Area', 'Fish / redd', 'FpR Std. Error', 'Prop. Hatchery', 'Prop Std. Error')) %>%
  kable_styling(bootstrap_options = c("striped", "hover"),
                latex_options = c("striped",
                                  "HOLD_position"))
...
low_met_spwn |>
  ...
  kableExtra::kbl(caption = "Estimates (CV) of spawners in lower Methow reaches by reach and origin.",
                  booktabs = T) |>
  kable_styling(bootstrap_options = c("striped", "hover"),
                latex_options = "striped") |>
...
spwn_strm |>
  ...
  pivot_wider(names_from = group,
              values_from = prnt_val,
              names_sort = T) |>
  kableExtra::kbl(caption = "Estimates (CV) of spawners by area and origin.",
                  booktabs = T,
...
read_excel(paste("T:/DFW-Team FP Upper Columbia Escapement - General/UC_Sthd/estimates",
                 ...
                 "UC_STHD_Model_Output_20241210.xlsx",
                 sep = "/"),
           "Sex Error Rates") |> 
  rename(`Lower CI` = LCI,
         `Upper CI` = UCI) |> 
...
## Estimates of Steelhead at Priest Rapids Dam {#appendB}
