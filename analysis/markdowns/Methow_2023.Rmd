...
redd_results <- results_lst |> 
  ...
  relocate(river, reach, location,
           .before = 1) |>
  # change where fish/redd etc. data comes from for reach MH1
  mutate(across(location,
                ~ case_when(. == "Tributaries" ~ as.character(river),
...
HOR-C origin steelhead PIT-tagged at Priest Rapids dam were not detected in Libby Creek this year, leading to estimates of zero spawners for those origins in that tributary (Table \@ref(tab:spwn-area)). Because these estimates are based on a sample of the entire Upper Columbia steelhead run, it's possible that a low number of HOR-C steelhead may have spawned in tat tributaries, but it's potentially more likely that the estimate of zero HOR-C steelhead spawners in those areas is correct.
...
read_excel("T:/DFW-Team FP Upper Columbia Escapement - General/UC_Sthd/estimates/UC_STHD_Model_Output_20240108.xlsx",
           "Sex Error Rates") |> 
  rename(`Lower CI` = LCI,
         `Upper CI` = UCI) |> 
  kable(digits = 3,
